...
def evaluate(model, dataloader):
    ...
    for item in dataloader:
        ...
        with torch.no_grad():
            ...
            loss = loss_fn(shifted_logits.view(-1, shifted_logits.shape[-1]), shifted_labels.view(-1))
...
def evaluate_from_dataset(args, model, tokenizer):
    ...
    dataset = load_dataset(args.dataset, ignore_verifications=True, split="train")

    dataset = dataset.train_test_split(train_size = args.proportion, shuffle=True)['train']
    ...
    dataloader = DataLoader(dataset, batch_size=args.batch_size, collate_fn=collate_dataset)
    total_nll, total_num_datapoints, total_num_correctly_predicted = evaluate(model, dataloader)
