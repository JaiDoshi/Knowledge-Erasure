...
def run_rmu(
    ...
    for epoch in range(args.num_epochs):
        ...
        for idx, (unlearn_batch, retain_batch) in enumerate(zip(cycle(forget_data_list[0]), cycle(retain_data_list[0]))):
            ...
            unlearn_batch = {k: v.to(updated_model.device) for k, v in unlearn_batch.items()}
            retain_batch = {k: v.to(updated_model.device) for k, v in retain_batch.items()}
            ...
            if args.verbose:
                ...
                frozen_forget_activations = forward_with_cache(frozen_model, unlearn_batch, module=frozen_module, no_grad=True).to(updated_model.device)
            ...
            if num_steps % args.save_every == 0:
                updated_model.save_pretrained(path + "_checkpoint_" + str(num_steps))
...
def get_args():
    ...
    parser.add_argument("--peft_model", action="store_true", help="Is the model to peform unlearning on a PEFT model")
    parser.add_argument(
    ...
    parser.add_argument(
        "--retain_corpus",
        type=str,
        help="Path to directory of retain corpus"
    )
    parser.add_argument(
        "--forget_corpus",
        type=str,
        help="Path to directory of forget corpus"
    )
    ...
    ### rmu hyperparameters
    parser.add_argument("--alpha", type=str, default="100", help="Retain weight")
    parser.add_argument(
        "--steering_coeff",
        type=str,
        default="20",
        help="Steer vector weight",
    )
    ...
    parser.add_argument("--save_every", type=int, default=50, help="How many steps to checkpoint the model")
    parser.add_argument(
        "--max_unlearn_steps",
        type=int,
        default=1000,
...
if __name__ == "__main__":
    ...
    frozen_model, tokenizer = load_model(args.normal_model_name_or_path, args.tokenizer_path, False)
    updated_model, tokenizer = load_model(args.model_name_or_path, args.tokenizer_path, args.peft_model)
